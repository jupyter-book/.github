## Community-driven tools for technical communication

📖 Jupyter Book is [community
developed](https://next.jupyterbook.org/about/community/) project that
empowers you to create documents and knowledge bases that are
reusable, reproducible, and interactive.

Jupyter Book, the organization, develops [Jupyter
Book](https://next.jupyterbook.org/): user-friendly software for
rapidly building rich, interlinked books and websites from [MyST
Markdown](https://next.jupyterbook.org/tutorial/mystmd/) content.

The organization also supports numerous other pieces of software
around which Jupyter Book is built, most notably the [MyST Document
Engine](https://mystmd.org/).

> [!TIP]
> Jupyter Book is distribution of the MyST Document Engine that
> includes configuration, plugins, and styles.
> This is version 2 of Jupyter Book.
> Version 1 was built around Sphinx.

### Related orgs and repositories

For historical reasons, our work is spread across multiple
organizations. We list them here, along with some key repositories, to
help contributors navigate:

- https://github.com/jupyter-book
  - [mystmd](https://github.com/jupyter-book/mystmd) — MyST document engine
  - [myst-theme](https://github.com/jupyter-book/myst-theme/) — MyST web renderer
  - [landing page example](https://github.com/jupyter-book/example-landing-pages) — example MyST book with landing page

- https://github.com/myst-templates
  - [templates](https://github.com/myst-templates/templates) — MyST template server
  - [book theme](https://github.com/myst-templates/book-theme) — MyST book renderer
  - [LaPreprint typst theme](https://github.com/myst-templates/lapreprint-typst) — MyST typst paper renderer

- https://github.com/executable-books
  - [myst-vs-code](https://github.com/executablebooks/myst-vs-code) — VS Code MyST highlighter
  - [github-activity](https://github.com/executablebooks/github-activity) — release note generator
